...
export default function ProgramsShow() {
    const { Program } = usePage<PageProps>().props

    if (!Program) {
        return (
            <AppLayout>
                <div className="p-6 text-red-600 dark:text-red-400">Program data not found.</div>
            </AppLayout>
        )
    }
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `Details ${Program.name}`, href: `/Programs/${Program.id}` },
    ]

    // قائمة حقول البيانات مع الأيقونات
    const fields = [
        { label: 'ID', value: Program.id, icon: IdentificationIcon },
        { label: 'English Name', value: Program.name, icon: BuildingOffice2Icon },
        ...(Program.name_ar ? [{ label: 'Arabic Name', value: Program.name_ar, icon: BuildingOffice2Icon }] : []),
        ...(Program.code ? [{ label: 'Code', value: Program.code, icon: CodeBracketIcon }] : []),
    ]
