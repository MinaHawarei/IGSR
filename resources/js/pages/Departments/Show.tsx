...
export default function DepartmentsShow() {
    ...
    if (!department) {
        return (
            <AppLayout>
                <div className="p-6 text-red-600 dark:text-red-400">Department data not found.</div>
            </AppLayout>
        )
    }
    ...
    const breadcrumbs: BreadcrumbItem[] = [
        ...
        { title: `Details ${department.name}`, href: `/departments/${department.id}` },
    ]

    // قائمة حقول البيانات مع الأيقونات
    const fields = [
        { label: 'ID', value: department.id, icon: IdentificationIcon },
        { label: 'English Name', value: department.name, icon: BuildingOffice2Icon },
        ...(department.name_ar ? [{ label: 'Arabic Name', value: department.name_ar, icon: BuildingOffice2Icon }] : []),
        ...(department.code ? [{ label: 'Code', value: department.code, icon: CodeBracketIcon }] : []),
    ]
